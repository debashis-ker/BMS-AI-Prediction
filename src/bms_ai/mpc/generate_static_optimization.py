...
class StaticTestDataFetcher:
    ...
    def fetch_schedule_data(self) -> List[Dict[str, Any]]:
        ...
        if schedule_data is None:
            ...
            schedule_data = fetch_movie_schedule(ticket=self.config.SCHEDULE_TICKET)
...
class MPCOptimizationRunner:
    ...
    def load_model(self):
        ...
        try:
            ...
            # Apply configured setpoints
            self.mpc_system.update_setpoints(
        ...
        for idx, row in df.iterrows():
            try:
                ...
                results.append({
                    ...
                    'TempSu': row.get('TempSu', np.nan),
                    'TempSp1_lag_10m': row.get('TempSp1_lag_10m', np.nan),
                    'SpTREff_lag_10m': row.get('SpTREff_lag_10m', np.nan),
                    ...
                    'HuR1': row.get('HuR1', np.nan),
                    'co2_load_category': result.get('co2_load_category', 'unknown'),
                    'co2_load_factor': result.get('co2_load_factor', np.nan),
                    'occupied_setpoint': self.config.OCCUPIED_SETPOINT,
                    'unoccupied_setpoint': self.config.UNOCCUPIED_SETPOINT,
                    'comfort_min': self.mpc_system.config.sp_min_occupied,
                    'comfort_max': self.mpc_system.config.sp_max_occupied,
                    'hour_sin': row.get('hour_sin', np.nan),
                    'hour_cos': row.get('hour_cos', np.nan),
                    'day_sin': row.get('day_sin', np.nan),
                    'day_cos': row.get('day_cos', np.nan),
                    'is_overnight_movie': row.get('is_overnight', False),
                    'is_precooling': row.get('is_precooling', False),
                    'time_remaining_min': row.get('time_remaining_min'),
                    'time_until_next_min': row.get('time_until_next_min'),
