...
from src.bms_ai.utils.cassandra_utils import DEFAULT_BUILDING_ID, fetch_all_ahu_historical_data, fetch_mahu_data, fetch_cassandra_data
...
def tune_best_parameters(train_data):
    ...
    for params in all_params:
        ...
        try:
            ...
            # df_cv = cross_validation(m, initial='10 days', period='14 days', horizon='1 day', parallel="processes")
...
def model_saving(raw_data, discovered_features):
    ...
    for feature, assets in master_model_package.items():
        output_model_path = f"artifacts/Anamoly_Prophet_Models/{feature}_package.joblib"
...
def training_pipeline(background_tasks: BackgroundTasks, building_id: str, ticket: str, software_id: str, account_id: str, search_tag_groups: List[List[str]], ticket_type: Optional[str]):
    try:
        ...
        to_date_str = now_utc.strftime('%Y-%m-%d %H:%M:%S.%f%z')
        from_date_str = (now_utc - pd.Timedelta(days=30)).strftime('%Y-%m-%d %H:%M:%S.%f%z')

        log.info(f"Fetching 3 months of training data from {from_date_str} to {to_date_str}")
        
        raw_records = fetch_cassandra_data(
            building_id=building_id or DEFAULT_BUILDING_ID,
            datapoints=discovered_features,
